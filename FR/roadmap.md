...
## Shoot-for-the-stars (post-boot)

**Emergent agent protocol over a shared framebuffer.** Two ClaudioOS agents, each with partial observability of a shared world, allowed to communicate only via 7×7 = 49-pixel glyphs written to a shared framebuffer region. No pretrained language, no tool schema — just the pixels. Reproduces the cyrilXBT / 2026-04-07 setup where agents spontaneously invented symbols, theories about symbols, and self-evaluated their own writing. **Deferred until bare-metal boot + OS fundamentals are solid — this is a bells-and-whistles feature, not a priority.** Value when we get there: ultra-low-bandwidth agent comms, learned-codec research for inter-agent messages, interpretability probe for our own fine-tuned models, and a hell of a demo once the OS actually boots on hardware.
