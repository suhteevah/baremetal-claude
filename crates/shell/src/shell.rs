...
impl Shell {
    ...
    pub fn new() -> Self {
        log::info!("shell: initializing ClaudioOS shell");
    ...
    fn execute_command(
        ...
        let pipeline = match Pipeline::parse(input) {
            Ok(p) => p,
            Err(e) => {
                let msg = format!("claudio-shell: parse error: {}", e);
                log::warn!("shell: {}", msg);
                return (msg, 1);
            }
        };
...
// requires careful lifetime management. In ClaudioOS, the actual LineReader
